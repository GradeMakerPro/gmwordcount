...
(function () {
    ...
    for (var i = 0; i < characterMap.length; ++i) {
      ...
      if (isWordBoundary(characterMap, i)) {
        ...
        if (
          ...
          (options.includePunctuation || !PUNCTUATION$1.test(ch) || ch === '&')
        ) {
          // Line changed by GradeMaker
  ...
  var getText = function (node, schema) {
    ...
    // Start of GradeMaker edits to handle equations and assets
    let shallow = false;
    while ((node = treeWalker.next(shallow))) {
      shallow = false;
      if (
  ...
  function Plugin(delay) {
    ...
    global.add('gmwordcount', function (editor) {
      var api = get(editor);
      register(editor, api);
